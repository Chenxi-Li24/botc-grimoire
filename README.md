...
- 三个官方脚本全收录:暗流涌动 / 黯月传奇 / 教派紫月,说书人可随时切换
- 说书人配置板子与人数(5~15),座位按环形排布
- 玩家扫码/输入名字加入,自选空座位入座(开局前可换座),身份存 localStorage,刷新不掉,断线自动重连
- 按官方配置表随机分配角色,支持脚本特殊规则:男爵 −2镇民/+2外来者、方古 −1镇民/+1外来者
...
- 三个脚本的角色文案均为初版速写,校对只改 `backend/app/roles.py`
- 状态存内存,服务器重启即重置(存档在路线图中)
- 醉鬼/疯子伪装(玩家看到错误角色)未实现,分配后说书人需口头告知
- 板子仅含三个基础官方脚本,旅行者(Travellers)未收录
